...
def check_params(req: dict):
    ...
    ref_audio: UploadFile = req.get("ref_audio")
    ...
    if ref_audio.content_type not in ["audio/wav", "audio/mpeg", "audio/ogg"]:
        return JSONResponse(
            status_code=400,
            content={
                "message": "Invalid audio format. Please upload a WAV, MP3, or OGG file."
            },
        )
    if text in [None, ""]:
...
async def tts_handle(req: dict):
    ...
    audio_ref = req.get("audio_ref")
    audio_ref_path = req.get("audio_ref_path")

    with open(audio_ref_path, "wb") as buffer:
        buffer.write(await audio_ref.read())
